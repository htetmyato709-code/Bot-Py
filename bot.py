...
async def add_group_manual(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text("⚠️ ကျေးဇူးပြု၍ `/addgp -100xxxxxxx` ပုံစံဖြင့် Group ID ကို ထည့်ပေးပါ။")
        return
    ...
    try:
        ...
        await update.message.reply_text(f"✅ Group ID `{group_id}` ကို Database ထဲသို့ အောင်မြင်စွာ ထည့်သွင်းပြီးပါပြီ။")
    except Exception as e:
...
async def post_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not groups:
        await update.message.reply_text("❌ Database ထဲတွင် Group မရှိသေးပါ။ `/addgp [ID]` ဖြင့် Group အရင်ထည့်ပါ။")
        is_posting = False
        return
